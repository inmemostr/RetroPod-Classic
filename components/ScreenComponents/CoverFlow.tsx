  ...
  if (songs.length === 0) {
    return (
      <div className="w-full h-full bg-black relative flex items-center justify-center">
        <p className="text-gray-400 text-sm">No Songs in Library</p>
      </div>
    );
  }

  const safeIndex = Math.min(selectedIndex, songs.length - 1);
  ...
  return (
    <div className="w-full h-full bg-black relative overflow-hidden perspective-800">
                ...
                return (
                    ...
                    >
                        ...
                        {song.coverUrl ? (
                          ...
                          <div className="w-full h-full bg-gradient-to-br from-gray-600 to-gray-800 border border-gray-700 shadow-2xl flex items-center justify-center">
        ...
        <div className="absolute bottom-8 left-0 right-0 text-center z-50">
            <h3 className="text-white font-bold text-sm tracking-wide drop-shadow-md">{songs[safeIndex]?.title}</h3>
            <p className="text-gray-400 text-xs">{songs[safeIndex]?.artist}</p>
        </div>
        
        <div className="absolute top-2 right-2 text-[10px] text-gray-500 font-mono">
            {safeIndex + 1} of {songs.length}
        </div>
    </div>
  );
};
